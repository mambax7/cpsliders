...
function cpslidersMetaKeywords($content)
{
...
function cpslidersMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function cpsliders_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = cpsliders_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function cpsliders_Filter($url, $type = '') {
